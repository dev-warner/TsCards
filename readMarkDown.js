    ...
    try {
        ...
        const categorys = posts
            .reduce((acc, post) => {
                ...
                const { links: link } = acc.find((target) => type === target.label);
                const category = link.find(cat => cat.label.toLowerCase().match(post.category.toLowerCase()));
...
const social = [
    {
        label: 'github',
        link: 'https://github.com/dev-warner/TsCards'
    },
    {
        label: 'stackoverflow',
